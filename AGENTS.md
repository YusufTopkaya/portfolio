...
- **Twingo Racer audio**: `components/retro/racer/audio.ts` synthesizes everything with raw Web Audio (no assets) — engine hum follows a 5-speed gearbox (`GEAR_TOPS`/`SHIFT_TIME` in `engine.ts`: RPM sweeps each gear, throttle lifts ~0.28 s on every shift), bandpass-noise brake screech and tire skid (`EngineState.skid`/`rpm01`/`gear`/`shiftT`/`braking` feed it), and a synthwave loop that adds layers by score (lead ≥25k, octave doubler ≥60k, double-time hats ≥100k). Three buses (music/engine/menu) sit under the master mute; a SETTINGS panel (title-screen chip + pause-menu option, `.racer-settings` in `globals.css`) sets each 0-10, persisted in `localStorage("twingo:vol")`. Braking also lights the car sprite's stop lamps (chase view, drawn after the sprite in `engine.ts` render).
